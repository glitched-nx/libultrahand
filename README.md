...
Erweiterte [**libtesla**](https://github.com/WerWolv/libtesla) (ursprünglich von [WerWolv](https://github.com/WerWolv)) + **libultra** Bibliotheken für die Overlay-Entwicklung auf der Nintendo Switch

![libultrahand Logo](.pics/libultrahand.png)

## Verwendung
### Überschreiben von Themes und Wallpapers

Um das Theme, das Wallpaper und/oder direkte Sprachübersetzungen für dein Overlay anzupassen, kannst du die Standardeinstellungen überschreiben, indem du die folgenden Zeilen zu deinem `Makefile` hinzufügst:
...
Benutzer können benutzerdefinierte Ultrahand `theme.ini` und `wallpaper.rgba` Dateien für das Overlay angeben, die sich auf der SD-Karte im Verzeichnis `/config/<OVERLAY_NAME>/` oder `{UI_OVERRIDE_PATH}` befinden.

#### **Fehlerbehebung**
**Hinweis:** Makefile-Direktiven müssen auch zu `CXXFLAGS` hinzugefügt werden.
Du kannst dies tun, indem du die Direktiven hinzufügst, bevor `CXXFLAGS := $(CFLAGS)` definiert wird, oder `CXXFLAGS += ...` einfügst.

Es gibt seltene Fälle, in denen das Theme und das Wallpaper immer noch nicht geladen werden. Dies kann damit zusammenhängen, wie die GUI-Klasse in einigen Projekten verwendet wird. Als Workaround kannst du versuchen, die Direktive `INITIALIZE_IN_GUI_DIRECTIVE` hinzuzufügen.

```
# Für Theme-/Wallpaper-Laden in GUI-Klassenmethode (zum Projekt hinzufügen, wenn das Theme nicht erscheint)
INITIALIZE_IN_GUI_DIRECTIVE := 1
CFLAGS += -DINITIALIZE_IN_GUI_DIRECTIVE=$(INITIALIZE_IN_GUI_DIRECTIVE)
```

Dieser Fix wird für viele Projekte funktionieren, aber andere Projekte mögen diese Direktive möglicherweise nicht oder benötigen sie überhaupt nicht, also verwende sie mit Bedacht.

---

### Überschreiben von Sprachen
Für die Sprachübersetzung muss `UI_OVERRIDE_PATH` definiert sein. Übersetzungen werden direkt in der `drawString`-Methode des Renderers durchgeführt. Direkte Zeichenfolgen können zu einer JSON-Datei hinzugefügt werden, die sich in `/config/<OVERLAY_NAME>/lang/` oder `{UI_OVERRIDE_PATH}/lang/` befindet.

JSON-Dateien müssen im ISO 639-1-Format benannt werden (en, de, fr, es, etc...) und werden nur entsprechend der aktuellen Sprache verwendet, die im Ultrahand Overlay `/config/ultrahand/config.ini` eingestellt ist.

Das Format für Sprach-JSONs ist wie folgt.
```json
{
  "English String": "Übersetzte Zeichenfolge",
  "Another String": "Eine weitere Übersetzung",
  ...
}
```
...
Ultrahand Overlay-Themenvariablen und Einstellungen für dein Overlay werden automatisch bei der Initialisierung gelesen. Die Implementierung des Themenladens ist derzeit innerhalb von `OverlayFrame` und `HeaderOverlayFrame` festgelegt.

Wenn du dein Projekt jedoch in einzelne Teile aufteilst, die nur `tesla.hpp` importieren und Elemente modifizieren, musst du möglicherweise `/libultrahand/libultra/include` am Anfang deiner `INCLUDES` in deiner Makefile deklarieren.

Wenn das immer noch nicht funktioniert, musst du möglicherweise diese Zeile irgendwo hinzufügen, damit das Thema auf dieses Element angewendet wird.

```cpp
tsl::initializeThemeVars(); // Variablen für Ultrahand-Themen initialisieren
```
...
Entwickler sollten die folgenden Bibliotheken in ihrer Makefile einfügen, wenn sie die volle `libultra`-Funktionalität in ihren Projekten wünschen.
...
ASSERT_FATAL(smInitialize()); // notwendig, um Probleme beim Einschlafen des Geräts zu vermeiden
```
...
```
Diese Optionen sind sowohl in CFLAGS als auch in CXXFLAGS vorhanden. Sie weisen den Compiler an, jede Funktion und jedes Datenelement in einen eigenen Abschnitt zu platzieren, was es dem Linker ermöglicht, ungenutzten Code leichter zu identifizieren und zu entfernen.

```
-Wl,--gc-sections
```
In LDFLAGS enthalten. Dieses Linker-Flag weist den Linker an, ungenutzte Abschnitte zu entfernen, die durch -ffunction-sections und -fdata-sections erstellt wurden. Dies stellt sicher, dass Funktionen oder Daten, die nicht verwendet werden, aus dem endgültigen ausführbaren Programm entfernt werden.
...
-fuse-linker-plugin
```
Dieses Flag ermöglicht eine bessere Zusammenarbeit zwischen Compiler und Linker bei der Verwendung von LTO, was weiter zur Optimierung und Eliminierung ungenutzten Codes beiträgt. Zusammen sorgen diese Flags (-ffunction-sections, -fdata-sections, -Wl,--gc-sections und -flto) dafür, dass ungenutzte Funktionen oder Daten während des Build-Prozesses entfernt werden, was zu einem kleineren und optimierten finalen Binary führt.
...
Beiträge sind willkommen! Wenn Du Ideen, Vorschläge oder Fehlerberichte hast, erstelle bitte ein [Issue](https://github.com/ppkantorski/libultrahand/issues/new/choose), reiche einen [Pull Request](https://github.com/ppkantorski/libultrahand/compare) ein oder kontaktiere mich direkt auf [GBATemp](https://gbatemp.net/threads/ultrahand-overlay-the-fully-craft-able-overlay-executor.633560/).

[![ko-fi](https://ko-fi.com/img/githubbutton_sm.svg)](https://ko-fi.com/X8X3VR194)

## Lizenz

Dieses Projekt ist lizenziert und verteilt unter [GPLv2](LICENSE) mit einer [benutzerdefinierten Bibliothek](libultra), die [CC-BY-4.0](SUB_LICENSE) verwendet.
